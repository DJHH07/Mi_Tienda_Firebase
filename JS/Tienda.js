...
const getTodoFromFirebase = async () => {
    try {
        const datos = await axios.get('https://crud-todo-ef720-default-rtdb.firebaseio.com/Todo.json')
...
const Pintar = () => {
    // cons esto obtenemos los valores para pintar
    getTodoFromFirebase().then(r =>{
        lista.innerHTML = ``
        console.log(r);
        r.forEach(i => {
...
const updateTodoFirebase = async (updateTodo) => {
    const data = await axios.patch(`https://crud-todo-ef720-default-rtdb.firebaseio.com/Todo/${updateTodo.id}.json`, updateTodo)
    // console.log(data)
    console.log("hola: ",updateTodo.id);
}
